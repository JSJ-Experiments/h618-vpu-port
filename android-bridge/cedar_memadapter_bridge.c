...
/*
 * ARMv7 Android/Bionic replacement for the vendor libMemAdapter.so.
 *
 * It deliberately uses the H618 port's per-file coherent-memory ABI rather
 * than Android ION.  The library is private deployment input: it is built
...
static struct AndroidVdecScMemOpsS g_vdec_memops = {
