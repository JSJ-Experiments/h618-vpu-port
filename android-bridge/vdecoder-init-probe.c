...
int main(void) {
 ...
 add_plugins_fn add_plugins; create_fn create; destroy_fn destroy; init_fn init;
 if(!lib){fprintf(stderr,"dlopen: %s\n",dlerror());return 1;}
 *(void **)(&add_plugins)=dlsym(lib,"AddVDPlugin"); *(void **)(&create)=dlsym(lib,"CreateVideoDecoder"); *(void **)(&destroy)=dlsym(lib,"DestroyVideoDecoder"); *(void **)(&init)=dlsym(lib,"InitializeVideoDecoder");
 if(!add_plugins||!create||!destroy||!init){fprintf(stderr,"missing API\n");return 2;}
 add_plugins();
