...
int main(int argc, char **argv) {
 ...
 if (argc != 2) { fprintf(stderr, "usage: %s annexb.h264\n", argv[0]); return 64; }
 ...
 *(void **)(&add_plugins)=dlsym(lib,"AddVDPlugin"); *(void **)(&create)=dlsym(lib,"CreateVideoDecoder"); *(void **)(&destroy)=dlsym(lib,"DestroyVideoDecoder"); *(void **)(&init)=dlsym(lib,"InitializeVideoDecoder"); *(void **)(&request_stream)=dlsym(lib,"RequestVideoStreamBuffer"); *(void **)(&submit_stream)=dlsym(lib,"SubmitVideoStreamData"); *(void **)(&decode)=dlsym(lib,"DecodeVideoStream"); *(void **)(&request_picture)=dlsym(lib,"RequestPicture"); *(void **)(&return_picture)=dlsym(lib,"ReturnPicture");
 ...
 add_plugins(); memset(&s,0,sizeof(s)); memset(&c,0,sizeof(c)); s.eCodecFormat=0x115; s.nWidth=320; s.nHeight=240; s.nFrameRate=30; s.nFrameDuration=33333; s.bIsFramePackage=0; c.eOutputPixelFormat=6; c.nVbvBufferSize=2*1024*1024; c.nFrameBufferNum=4;
 ...
 for(i=0;i<100;i++){ void *pic; rc=decode(d,i>50,0,0,0); pic=request_picture(d,0); if(pic){return_picture(d,pic); printf("Android CedarX H264 hardware decode OK (result=%d)\n",rc);destroy(d);free(src);return 0;} }
