				  ...
				  struct platform_device *platform_dev)
{
	...
	cedrus_dev->dma_offset = variant->dma_offset;
	cedrus_dev->enc_addr_shift = variant->enc_addr_shift;
...
static int cedrus_remove(struct platform_device *platform_dev)
{
...
/*
 * H616 and H618 expose the newer VE revision through the same DT binding.
 * Decode capabilities match the Orange Pi 6.1 downstream driver.  H.264
 * encode is kept behind this experimental module so the stock module remains
 * the boot-safe fallback while the H618 register differences are validated.
 */
static const struct cedrus_variant cedrus_variant_sun50i_h616 = {
	.capabilities	= CEDRUS_CAPABILITY_UNTILED |
			  CEDRUS_CAPABILITY_MPEG2_DEC |
			  CEDRUS_CAPABILITY_H264_DEC |
			  CEDRUS_CAPABILITY_H265_DEC |
...
static const struct of_device_id cedrus_of_match[] = {
	...
	},
	{
		...
		.data		= &cedrus_variant_sun50i_h616,
	},
	{ /* sentinel */ }
};
