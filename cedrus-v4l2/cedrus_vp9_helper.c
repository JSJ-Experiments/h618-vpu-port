 ...
 *
 * Embedded with Cedrus-local symbol names because the Orange Pi 6.1 BSP does
 * not enable CONFIG_V4L2_VP9. Keep this file in sync with the kernel helper.
 */
...
const u8 cedrus_vp9_kf_y_mode_prob[10][10][9] = {
	{
		/* above = dc */
...
const u8 cedrus_vp9_kf_partition_probs[16][3] = {
	/* 8x8 -> 4x4 */
...
const u8 cedrus_vp9_kf_uv_mode_prob[10][9] = {
...
const struct v4l2_vp9_frame_context cedrus_vp9_default_probs = {
	.tx8 = {
		{ 100 },
		{  66 },
	},
...
void cedrus_vp9_fw_update_probs(struct v4l2_vp9_frame_context *probs,
			      const struct v4l2_ctrl_vp9_compressed_hdr *deltas,
			      const struct v4l2_ctrl_vp9_frame *dec_params)
{
...
u8 cedrus_vp9_reset_frame_ctx(const struct v4l2_ctrl_vp9_frame *dec_params,
			    struct v4l2_vp9_frame_context *frame_context)
{
	...
	if (dec_params->flags & V4L2_VP9_FRAME_FLAG_KEY_FRAME ||
		...
		if (dec_params->flags & V4L2_VP9_FRAME_FLAG_KEY_FRAME ||
			...
			for (i = 0; i < 4; ++i)
				/* save_probs(i) */
				memcpy(&frame_context[i], &cedrus_vp9_default_probs,
				       sizeof(cedrus_vp9_default_probs));
		else if (dec_params->reset_frame_context == V4L2_VP9_RESET_FRAME_CTX_SPEC)
			/* save_probs(fctx_idx) */
			memcpy(&frame_context[fctx_idx], &cedrus_vp9_default_probs,
			       sizeof(cedrus_vp9_default_probs));
		fctx_idx = 0;
	}
...
void cedrus_vp9_adapt_coef_probs(struct v4l2_vp9_frame_context *probs,
...
void cedrus_vp9_adapt_noncoef_probs(struct v4l2_vp9_frame_context *probs,
				  struct v4l2_vp9_frame_symbol_counts *counts,
...
bool
cedrus_vp9_seg_feat_enabled(const u8 *feature_enabled,
