...
int cedrus_enc_format_coded_configure(struct cedrus_context *ctx)
{
	...
	if (dev->enc_addr_shift)
		/* Captured from a successful H618 Android VENC job. */
		value = VE_MODE_RAMPD | BIT(30) | VE_MODE_DDR_MODE_BW_256 |
			VE_MODE_ENC_ENABLE | VE_MODE_ENC_ISP_ENABLE |
...
int cedrus_enc_format_picture_configure(struct cedrus_context *ctx)
{
	...
	if (dev->enc_addr_shift) {
		/* H618 expresses ISP dimensions in 8x8 blocks. */
		width_mbs = DIV_ROUND_UP(pix_format->width, 8);
		height_mbs = DIV_ROUND_UP(pix_format->height, 8);
		cedrus_write(dev, VE_ISP_PIC_INFO_REG,
			     (width_mbs << 16) | height_mbs);
		cedrus_write(dev, VE_ISP_SCALER_SIZE_REG,
			     (height_mbs << 16) | width_mbs);
	} else {
	...
	if (dev->enc_addr_shift)
		cedrus_write(dev, VE_ISP_H618_CTRL_REG, 0x42);

	/* Address */

	cedrus_job_buffer_picture_dma(ctx, &luma_addr, &chroma_addr);

	cedrus_write(dev, VE_ISP_INPUT_LUMA_ADDR_REG,
		     cedrus_dma_addr(dev, luma_addr));
	cedrus_write(dev, VE_ISP_INPUT_CHROMA0_ADDR_REG,
		     cedrus_dma_addr(dev, chroma_addr));
