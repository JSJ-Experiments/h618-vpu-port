...
static void cedrus_enc_h264_coded_nalu_start(struct cedrus_device *dev,
					     u8 header)
{
	/* Annex-B framing is outside the RBSP and must never be escaped. */
	cedrus_enc_h264_coded_eptb(dev, false);
	cedrus_enc_h264_coded_u32(dev, 0x1);
	cedrus_enc_h264_coded_u8(dev, header);

	/* Escape only the RBSP payload which follows the NAL header. */
	cedrus_enc_h264_coded_eptb(dev, true);
}
...
static void cedrus_enc_h264_job_configure_sps(struct cedrus_context *cedrus_ctx)
{
	...
	u32 coded_width = h264_ctx->width_mbs * 16;
	u32 coded_height = h264_ctx->height_mbs * 16;
	...
	cedrus_enc_h264_coded_nalu_start(dev, header);
	...
	if (selection->left || selection->top ||
	    selection->width != coded_width ||
	    selection->height != coded_height) {
		crop_left = selection->left;
		crop_right = coded_width - selection->width -
			     selection->left;
		crop_top = selection->top;
		crop_bottom = coded_height - selection->height -
			      selection->top;
	...
	cedrus_enc_h264_coded_u32(dev, timeperframe->numerator);

	/* A frame requires two ticks in H.264. */
	/* Syntax element: time_scale. */
	cedrus_enc_h264_coded_u32(dev, timeperframe->denominator * 2);
...
static int cedrus_enc_h264_job_configure(struct cedrus_context *cedrus_ctx)
{
	...
	if (dev->enc_addr_shift)
		cedrus_write(dev, VE_ENC_AVC_PIC_INFO_REG,
			     (DIV_ROUND_UP(pix_format->width, 8) << 16) |
			     DIV_ROUND_UP(pix_format->height, 8));
	...
	cedrus_write(dev, VE_ENC_AVC_MB_INFO_ADDR_REG,
		     cedrus_enc_dma_addr(dev, h264_ctx->mb_info_dma));
	...
	cedrus_write(dev, VE_ENC_AVC_REC_ADDR_Y_REG,
		     cedrus_enc_dma_addr(dev, h264_buffer->rec_dma));
	cedrus_write(dev, VE_ENC_AVC_REC_ADDR_C_REG,
		     cedrus_enc_dma_addr(dev, h264_buffer->rec_dma +
	...
	cedrus_write(dev, VE_ENC_AVC_REF0_ADDR_Y_REG,
		     cedrus_enc_dma_addr(dev, addr));
	cedrus_write(dev, VE_ENC_AVC_REF0_ADDR_C_REG,
		     cedrus_enc_dma_addr(dev, addr + luma_size));
	...
	cedrus_write(dev, VE_ENC_AVC_SUBPIX_ADDR_NEW_REG,
		     cedrus_enc_dma_addr(dev, h264_buffer->subpix_dma));
	...
	cedrus_write(dev, VE_ENC_AVC_SUBPIX_ADDR_LAST_REG,
		     cedrus_enc_dma_addr(dev, h264_ctx->subpix_last_dma));
	...
	value = VE_ENC_AVC_PARA1_QP_CHROMA_OFFSET0(job->chroma_qp_index_offset) |
		VE_ENC_AVC_PARA1_STRIDE_MBS_DIV_48(stride_mbs_div_48) |
		VE_ENC_AVC_PARA1_RC_MODE_FIXED |
		VE_ENC_AVC_PARA1_FIXED_QP(job->qp);
	if (dev->enc_addr_shift)
		value |= VE_ENC_AVC_PARA1_H618_MODE |
			 VE_ENC_AVC_PARA1_DYNAMIC_ME_EN;
	cedrus_write(dev, VE_ENC_AVC_PARA1_REG, value);

	cedrus_write(dev, VE_ENC_AVC_PARA2_REG, 0);

	if (dev->enc_addr_shift) {
		/* Defaults captured from the Android 12 H618 VENC at 320x240. */
		cedrus_write(dev, VE_ENC_AVC_DYNAMIC_ME_PAR0_REG, 0x00090001);
		cedrus_write(dev, VE_ENC_AVC_DYNAMIC_ME_PAR1_REG, 0x00300027);
	} else {
		/* Dynamic motion estimation is disabled on the older encoder. */
	...
	if (dev->enc_addr_shift) {
		cedrus_write(dev, VE_ENC_AVC_RC_INIT_REG, 0x80001900);
		cedrus_write(dev, VE_ENC_AVC_RC_MAD_TH0_REG, 0x120f0c09);
		cedrus_write(dev, VE_ENC_AVC_RC_MAD_TH1_REG, 0x261e1814);
		cedrus_write(dev, VE_ENC_AVC_RC_MAD_TH2_REG, 0xffff3630);
		cedrus_write(dev, VE_ENC_AVC_RC_MAD_TH3_REG, 0x00001c20);
	} else {
	...
	if (dev->enc_addr_shift)
		cedrus_write(dev, VE_ENC_AVC_ME_PARA_REG,
			     VE_ENC_AVC_ME_PARA_QUANT_THRESHOLD_DIS |
			     VE_ENC_AVC_ME_PARA_SKIP_MB_DIS);
	else
		cedrus_write(dev, VE_ENC_AVC_ME_PARA_REG,
	...
	if (dev->enc_addr_shift)
		cedrus_write(dev, VE_ENC_AVC_H618_CTRL_REG, 0x12222);
...
static void cedrus_enc_h264_job_trigger(struct cedrus_context *ctx)
{
	...
	int_value = VE_ENC_AVC_INT_EN_STALL |
		    VE_ENC_AVC_INT_EN_FINISH;
	if (dev->enc_addr_shift)
		int_value |= VE_ENC_AVC_INT_EN_MB_OVERTIME |
			     (DIV_ROUND_UP(pix_format->width, 80) << 24) |
			     (DIV_ROUND_UP(pix_format->width, 16) << 16);
	cedrus_write(dev, VE_ENC_AVC_INT_EN_REG, int_value);
...
static void cedrus_enc_h264_job_finish(struct cedrus_context *ctx, int state)
{
	...
	if (WARN_ON(length > vb2_plane_size(vb2_buffer, 0))) {
		/* Never publish a hardware-reported length outside the DMA buffer. */
		vb2_set_plane_payload(vb2_buffer, 0, 0);
...
static const struct v4l2_frmsize_stepwise cedrus_enc_h264_frmsize = {
	.min_width	= 16,
	/*
	 * Keep the initial H618 hardware validation deliberately small.  This is
	 * raised in controlled steps after each size has passed stream and decode
	 * validation; it prevents an accidental 4K job while this backport is
	 * still experimental.
	 */
	.max_width	= 3840,
	.step_width	= 2,
