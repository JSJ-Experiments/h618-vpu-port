...
enum cedrus_capability {
	...
	CEDRUS_CAPABILITY_JPEG_ENC	= BIT(8),
};
...
/* Translate the CPU physical DMA address into the VE bus address. */
static inline u32 cedrus_dma_addr(struct cedrus_device *dev, dma_addr_t addr)
{
	if (!addr)
		return 0;

	if (WARN_ON(addr < dev->dma_offset ||
		    addr - dev->dma_offset > U32_MAX))
		return 0;

	return (u32)(addr - dev->dma_offset);
}

/* Newer (H616/H618) VENC registers store DMA addresses in 256-byte units.
 * Decoder registers retain the existing byte-address ABI. */
static inline u32 cedrus_enc_dma_addr(struct cedrus_device *dev,
				      dma_addr_t addr)
{
	if (!addr)
		return 0;
	if (!dev->enc_addr_shift)
		return cedrus_dma_addr(dev, addr);

	if (WARN_ON((addr >> dev->enc_addr_shift) > U32_MAX))
		return 0;

	return lower_32_bits(addr >> dev->enc_addr_shift);
}
